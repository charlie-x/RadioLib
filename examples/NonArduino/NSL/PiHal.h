...
#define LG_LOW            (0)
#define LG_HIGH           (1)
#define LG_RISING_EDGE    (2)
#define LG_FALLING_EDGE    (3)

void dumphex ( unsigned char *string, size_t length )
{
...
int lgGpioWrite( int handle, int pin, int value)
{
  printf("lgGpioWrite %d %d %d\n", handle, pin, value);
  return 1;
}

int lgGpioFree( int handle, int num)
{
  printf("lgGpioFree\n");
  return 0;
}
...
int lgSpiOpen( int handle, int channel, unsigned long speed, int flags)
{
  printf("lgSpiOpen %d %d %lu %d\n", handle, channel, speed, flags);
  return 2;
}
...
int lgSpiXfer( int handle,char *in, char *out , int length)
{
  printf("lgSpiXfer %d  %d\n\t" , handle, length);
  dumphex((unsigned char *)in, length);
  return 0;
}

int lgGpiochipClose( int handle)
{
  printf("lgGpiochipClose\n");
  return 0;
}

int lgGpioClaimInput( int handle, int input, int pin)
{
  printf("lgGpioClaimInput %d %d %d\n", handle, input, pin);
  return 0;
}

int lgGpioClaimOutput( int handle, int flags, int pin, int state)
{
  printf("lgGpioClaimOutput %d %d %d %d\n",handle, flags, pin  , state);
  return 0;
}

char *lguErrorText( int error)
{
  printf("lguErrorText\n");
  return "";
}

int lgTxPwm(int handle, int pin, int freq, int fifty, int zero, int duration)
{
  printf("lgTxPwm\n");
  return 0;
}
...
class PiHal : public RadioLibHal {
  public:
    ...
    void pinMode(uint32_t pin, uint32_t mode) override {
      ...
      switch(mode) {
        ...
        case PI_OUTPUT:
          ...
          break;
        default:
          fprintf(stderr, "Unknown pinMode mode %u \n", mode);
          return;
      }

      if(result < 0) {
        fprintf(stderr, "Could not claim pin %i for mode %i: %s\n",
    ...
    void digitalWrite(uint32_t pin, uint32_t value) override {
      ...
      if(result < 0) {
        fprintf(stderr, "Error writing value to pin %i: %s\n", pin, lguErrorText(result));
      }
    }
    ...
    uint32_t digitalRead(uint32_t pin) override {
      ...
      if(result < 0) {
        fprintf(stderr, "Error writing reading from pin %i : %s\n", pin, lguErrorText(result));
      }
      return result;
    }
